...
def reverse_transform(arr, search_space):
    param_dict = {param.name: (int(val) if isinstance(param, UniformInt)
                               else param.values[int(val)] if isinstance(param, Choice)
                               else val)
                  for val, param in zip(arr, search_space.values())}
    return param_dict
